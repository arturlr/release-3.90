    ...
    public partial class BackwardCompatibility1XController : BasePublicController
    {
        ...
        public virtual IActionResult GeneralRedirect()
        {
            ...
            switch (aspxfileName)
            {
                ...
                case "product":
                    {
                        return RedirectProduct(Request.Query["productid"].ToString(), false);
                    }
                case "category":
                    {
                        return RedirectCategory(Request.Query["categoryid"].ToString(), false);
                    }
                case "manufacturer":
                    {
                        return RedirectManufacturer(Request.Query["manufacturerid"].ToString(), false);
                    }
                case "producttag":
                    {
                        return RedirectProductTag(Request.Query["tagid"].ToString(), false);
                    }
                case "news":
                    {
                        return RedirectNewsItem(Request.Query["newsid"].ToString(), false);
                    }
                case "blog":
                    {
                        return RedirectBlogPost(Request.Query["blogpostid"].ToString(), false);
                    }
                case "topic":
                    {
                        return RedirectTopic(Request.Query["topicid"].ToString(), false);
                    }
                case "profile":
                    {
                        return RedirectUserProfile(Request.Query["UserId"].ToString());
                    }
