...
namespace Nop.Web.Framework
{
    ...
    public partial class RemotePost
    {
        ...
        public async Task PostAsync()
        {
            ...
            if (NewInputForEachValue)
            {
                foreach (string key in _inputValues.Keys)
                {
                    ...
                    if (values != null)
                    {
                        ...
                        {
                            sb.AppendFormat("<input name=\"{0}\" type=\"hidden\" value=\"{1}\">", WebUtility.HtmlEncode(key), WebUtility.HtmlEncode(value));
                        }
                    }
                }
            }
            else
            {
                for (int i = 0; i < _inputValues.Keys.Count; i++)
                    sb.AppendFormat("<input name=\"{0}\" type=\"hidden\" value=\"{1}\">", WebUtility.HtmlEncode(_inputValues.Keys[i]), WebUtility.HtmlEncode(_inputValues[_inputValues.Keys[i]]));
            }
            sb.Append("</form>");
            sb.Append("</body></html>");
        ...
        /// <summary>
        /// Post (synchronous wrapper for backward compatibility)
        /// </summary>
        public void Post()
        {
