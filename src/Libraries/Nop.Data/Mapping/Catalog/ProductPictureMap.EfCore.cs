...
namespace Nop.Data.Mapping.Catalog
{
    public partial class ProductPictureMap : NopEntityTypeConfiguration<ProductPicture>
    {
        public override void Configure(EntityTypeBuilder<ProductPicture> builder)
        {
            ...
            builder.HasOne(pp => pp.Product).WithMany(p => p.ProductPictures).HasForeignKey(pp => pp.ProductId).IsRequired();
            builder.HasOne(pp => pp.Picture).WithMany().HasForeignKey(pp => pp.PictureId).IsRequired();
