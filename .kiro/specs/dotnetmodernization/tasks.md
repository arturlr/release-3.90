...
- [ ] 7. Port Nop.Web to ASP.NET Core
  ...
  - [ ] 7.7 Smoke-check the ASP.NET Core host
    ...
    - **PRIORITISED LIST FROM TASKS 7.3 AND 7.4:** (1) the home page, which fans out to ~15 `@Html.Action` child actions through 7.3's bridge; (2) a product slug URL plus a view that calls `Url.RouteUrl("Product", …)`, exercising `SlugRouteTransformer` and the seven `SuppressMatchingMetadata` routes; (3) `@Html.Widget` and an empty widget zone; (4) that `NopConfig` reaches the Autofac singleton so `RedisConnectionWrapper`/`AzurePictureService`/`UserAgentHelper` see authored values; (5) `IUserAgentHelper.IsSearchEngine()`, newly live as of 7.4 — the first call parses the 46 MB `browscap.xml` and **writes** `App_Data/browscap.crawlersonly.xml`, so confirm the directory is writable and the first request is not pathologically slow; (6) that emitted `<link>`/`<script>` URLs carry `?v=` **and** return 200; (7) that `GET /App_Data/Settings.txt` returns 404. Also still open for a real database: deferrals 7.2-1 and 4.11. See runtime-deferrals.md §30 and §36.
...
- [ ] 8. Port Nop.Admin to ASP.NET Core
  ...
  - [ ] 8.3 Convert all Nop.Admin controllers to ASP.NET Core MVC
    ...
    - **ALSO FROM 7.3 (deferral 7.3-1):** `Nop.Web/Extensions/ChildActionExtensions.cs` (the `Html.Action` bridge) is needed by the admin views too. Recommend **promoting it to `Nop.Web.Framework`** rather than duplicating it; the plugins then get it for free.
