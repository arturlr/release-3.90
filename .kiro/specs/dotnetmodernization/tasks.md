...
- [ ] 8. Port Nop.Admin to ASP.NET Core
  ...
  - [ ] 8.2 Replace the Admin AreaRegistration with ASP.NET Core area routing
    ...
    - **ALSO FROM 8.1:** 33 distinct `~/Admin/…` literals in the admin views hard-code the `Admin` route prefix that `MapAreaControllerRoute` must preserve. They were catalogued by 8.1's audit (runtime-deferrals.md §47) but not touched.
